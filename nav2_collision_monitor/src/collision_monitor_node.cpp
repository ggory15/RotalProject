...
: nav2_util::LifecycleNode("collision_monitor", "", options),
  ...
  node_name_("collision_monitor")
{
}

CollisionMonitor::CollisionMonitor(const std::string & name, const rclcpp::NodeOptions & options)
      ...
      if (polygon_type == "polygon") {
        polygons_.push_back(
          std::make_shared<Polygon>(
            node, polygon_name, tf_buffer_, base_frame_id, transform_tolerance, node_name_));
    ...
    for (std::string source_name : source_names) {
      ...
      if (source_type == "scan") {
        std::shared_ptr<Scan> s = std::make_shared<Scan>(
          ...
          transform_tolerance, source_timeout, node_name_);
