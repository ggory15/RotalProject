...
Polygon::Polygon(
  ...
  const tf2::Duration & transform_tolerance, const std::string & node_name)
...
{ 
  RCLCPP_INFO(rclcpp::get_logger(node_name_), "[%s]: Creating Polygon", polygon_name_.c_str());
}

Polygon::~Polygon()
{
  RCLCPP_INFO(rclcpp::get_logger(node_name_), "[%s]: Destroying Polygon", polygon_name_.c_str());
  poly_.clear();
}
...
bool Polygon::getActivation() const
{
  return activate_;
}
...
void Polygon::publish()
{
  ...
  activate_ =  node->get_parameter(polygon_name_ + ".activate").as_bool();
...
bool Polygon::getCommonParameters(std::string & polygon_pub_topic)
{
  ...
  try {
    ...
    nav2_util::declare_parameter_if_not_declared(
      node, polygon_name_ + ".activate", rclcpp::ParameterValue(true));
    activate_ = node->get_parameter(polygon_name_ + ".activate").as_bool();
