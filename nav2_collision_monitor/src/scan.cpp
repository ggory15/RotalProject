...
: Source(
  ...
  data_(nullptr)
{
  RCLCPP_INFO(rclcpp::get_logger(node_name_), "[%s]: Creating Scan", source_name_.c_str());
}

Scan::~Scan()
{
  RCLCPP_INFO(rclcpp::get_logger(node_name_), "[%s]: Destroying Scan", source_name_.c_str());
  data_sub_.reset();
}
