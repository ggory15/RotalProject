...
: Source(
    ...
    transform_tolerance, source_timeout, node_name),
  data_(nullptr)
{
  RCLCPP_INFO(rclcpp::get_logger(node_name_), "[%s]: Creating PointCloud", source_name_.c_str());
}

PointCloud::~PointCloud()
{
  RCLCPP_INFO(rclcpp::get_logger(node_name_), "[%s]: Destroying PointCloud", source_name_.c_str());
  data_sub_.reset();
}
