...
: Polygon::Polygon(node, polygon_name, tf_buffer, base_frame_id, transform_tolerance, node_name)
{

  RCLCPP_INFO(rclcpp::get_logger(node_name_), "[%s]: Creating Circle", polygon_name_.c_str());
}

Circle::~Circle()
{
  RCLCPP_INFO(rclcpp::get_logger(node_name_), "[%s]: Destroying Circle", polygon_name_.c_str());
}
