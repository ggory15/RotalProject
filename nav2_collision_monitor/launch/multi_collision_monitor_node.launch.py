...
def generate_launch_description():
    ...
    # Constant parameters
    lifecycle_nodes = ['collision_monitor', 'elevator_monitor']
    autostart = True
    ...
    declare_params_file2_cmd = DeclareLaunchArgument(
        'params_file2',
        default_value=os.path.join(package_dir, 'params', 'elevator_monitor_params.yaml'),
        description='Full path to the ROS2 parameters file to use for all launched nodes')
    ...
    start_elevator_monitor_cmd = Node(
            package='nav2_collision_monitor',
            executable='elevator_monitor',
            output='screen',
            emulate_tty=True,  # https://github.com/ros2/launch/issues/188
    ...
    ld.add_action(declare_params_file2_cmd)
    
    # Node launching commands
    ld.add_action(start_lifecycle_manager_cmd)
    ld.add_action(start_collision_monitor_cmd)
    ld.add_action(start_elevator_monitor_cmd)
