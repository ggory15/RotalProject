...
#include <kjj_localization/Util.h>
#include <kjj_localization/Convert.h>
#include <kjj_localization/Solver.h>
#include <kjj_localization/GridMap.h>
 ...
 *
 */
class KJJLocalizationNode : public rclcpp::Node {
public:
	KJJLocalizationNode(): Node("kjj_localization_node")
	{
		...
		m_map_update_thread = std::thread(&KJJLocalizationNode::update_loop, this);

		m_tf_broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(this);

		m_sub_scan_topic = this->create_subscription<sensor_msgs::msg::LaserScan>(m_scan_topic, rclcpp::SensorDataQoS(), std::bind(&KJJLocalizationNode::scan_callback, this, _1));
		m_sub_map_topic = this->create_subscription<nav_msgs::msg::OccupancyGrid>("/map", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable(), std::bind(&KJJLocalizationNode::map_callback, this, _1));
		m_sub_pose_estimate = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(m_initial_pose, 1, std::bind(&KJJLocalizationNode::pose_callback, this, _1));
...
protected:
	...
	std::vector<scan_point_t> convert_scan(const sensor_msgs::msg::LaserScan::SharedPtr scan, const Matrix<double, 4, 4>& odom_to_base)
	{
		...
		try {
			...
			RCLCPP_WARN_STREAM(this->get_logger(), "KJJLocalizationNode: lookupTransform(scan->header.frame_id, m_base_frame) failed: " << ex.what());
			return points;
		}
		try {
			...
			RCLCPP_WARN_STREAM(this->get_logger(), "KJJLocalizationNode: lookupTransform(m_base_frame, m_odom_frame) failed: " << ex.what());
			return points;
		}
...
// Here
	void loc_update()
	{
		...
		try {
			...
			RCLCPP_WARN_STREAM(this->get_logger(), "KJJLocalizationNode: lookup Transform(m_base_frame, m_odom_frame) failed: " << ex.what());
			return;
		}
		...
		{
			RCLCPP_WARN_STREAM(this->get_logger(),"KJJLocalizationNode: Number of points too low: " << points.size());
			return;
		}
    ...
    if(m_broadcast_info == true) {
      if(update_counter++ % 10 == 0) {
      	RCLCPP_INFO_STREAM(this->get_logger(),  "KJJLocalizationNode: score=" << float(best_score) << ", grad_uvw=[" << float(grad_std_uvw[0]) << ", " << float(grad_std_uvw[1])
	...
	{
		{
			...
			if(pose->header.frame_id != m_map_frame) {
				RCLCPP_WARN_STREAM(this->get_logger(), "KJJLocalizationNode: Invalid pose estimate frame");
				return;
			}
			...
			RCLCPP_INFO_STREAM(this->get_logger(), "KJJLocalizationNode: Got new map pose estimate: x=" << map_pose.getOrigin()[0]
	...
	void map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr ros_map)
	{
		...
		RCLCPP_INFO_STREAM(this->get_logger(), "KJJLocalizationNode: Got new map with dimensions " << ros_map->info.width << " x " << ros_map->info.height
	...
	void update_loop()
	{
		RCLCPP_WARN_STREAM(this->get_logger(),"KJJLocalizationNode: update_map() failed:");
...
int main(int argc, char** argv)
{
	...
	try {
		auto nh = std::make_shared<KJJLocalizationNode>() ;
		rclcpp::spin(nh);
	}
	catch(const std::exception& ex) {
		std::cout<<"KJJLocalizationNode: " << ex.what() << std::endl;
		return -1;
	}
